...
class GeneralApp extends StatefulWidget {
  ...
  const GeneralApp({Key? navKey}) : super(key: navKey);
  @override
...
class _GeneralAppState extends State<GeneralApp> {
  ...
  addItemDialog() async {
    return await showDialog(
      context: GeneralApp.navKey.currentState?.overlay?.context ?? context,
  ...
  Widget build(BuildContext context) {
                    ...
                    addItemDialog().then((value) async {
                      if (value != null &&
                          editableItems.indexWhere(
                              ...
                              -1) {
                        Navigator.pop(
                            GeneralApp.navKey.currentState?.overlay?.context ??
                                context);
                        updateParent(value);
                      }
                    });
      ...
      "Single dialog dark mode": Card(
        ...
        child: SearchChoices.single(
          ...
          closeButton: TextButton(
            onPressed: () {
              Navigator.pop(
                  GeneralApp.navKey.currentState?.overlay?.context ?? context);
            },
            child: Text(
              "Close",
              style: TextStyle(color: Colors.white),
    ...
    return MaterialApp(
      navigatorKey: GeneralApp.navKey,
      home: asTabs
